...
?>
<?php	   
	$sql_reg="select * from  tbl_std_reg where reg_id=(select max(reg_id) from tbl_std_reg)";
	$result_reg=mysqli_query($conn, $sql_reg);
	($data_reg=mysqli_fetch_array($result_reg))	
?>
<!DOCTYPE html>
<html lang="en">  
  <head>
  ...
  <form name="reg" method="POST" action="" onSubmit="return validateRegForm()">
    <div class="container">
      ...
      <!--footer content-->    
        <hr>
        <footer class="footer">
        <p style="font-family:bold; font-size:16px">Copyright &copy Deepak Corporation. All right reserved.</p>
      </footer>
